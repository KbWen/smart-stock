                ...
                <table className="w-full text-left text-sm">
                    <thead>
                        <tr className="border-b border-dark-border text-[11px] font-bold uppercase tracking-wider text-dark-muted">
                            ...
                            <th className="px-4 py-3 text-right">淨區間回報 (Net ROI)</th>
                            <th className="px-4 py-3 text-right text-sniper-gold">AI 機率</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-dark-border/50">
                        {picks.map((pick, idx) => {
                            const netRet = pick.net_return !== undefined ? pick.net_return : pick.actual_return
                            return (
                                <tr key={`${pick.ticker}-${idx}`} className="hover:bg-white/5 transition-colors">
                                    ...
                                    <td className={`px-4 py-4 text-right font-mono font-bold ${netRet >= 0 ? 'text-red-500' : 'text-sniper-green'}`}>
                                        <div>{(netRet * 100).toFixed(2)}%</div>
                                        {pick.net_return !== undefined && (
                                            <div className="text-[10px] text-dark-muted font-normal">
                                                原始: {(pick.actual_return * 100).toFixed(2)}%
                                            </div>
                                        )}
                                    </td>
